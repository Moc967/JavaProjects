...
public class CatalogoView {
    ...
    private boolean showMenu() {
        ...
        switch (opcao) {
            ...
            case "L":
                ...
                break;
            case "X":
                System.out.println("\nObrigado por utilizar nosso Catalogo!!");
                return false;
            default:
    ...
    private void edit() {
        ...
        if (nova != null && !editFlag) {
            while (true) {
                Receita other = controller.getReceita(nova.getNome());
                if (other != null && editFlag) {
                    String name = ConsoleUtils.getUserOption("Esta receita já existe!!%nPor favor, informe novo nome:%n");
                    if (!name.isBlank()) {
                        nova.setNome(name);
                    }
                } else {
